  ...
  _getAddBtnView(){
    return RaisedButton(
        onPressed: (){
          ...
          _add(_dbName, sql, name: _nameController.text, age: _ageController.text);
...
//          _delete(_dbName, sql);
          _delete2(_dbName, 'student_table');
        },
...
//          _update(_dbName, sql,[_nameController.text,1]);
          _update2(_dbName, 'student_table',{'name': _nameController.text, 'age': _ageController.text});
        },
...
//          _query(_dbName, sql);
  ...
  ///增
  _add(String dbName,String sql, {String name, String age}) async {
    //获取数据库路径
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
    print("数据库路径：$path");
    ...
    await db.transaction((txn) async {
      int count = await txn.insert('student_table', {'name': name, 'age': int.parse(age)});
    });
    await db.close();
  ...
  _delete2(String dbName,String table) async {
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
  
    Database db = await openDatabase(path);
    int count = await db.delete(table, where: null);
    await db.close();
  ...
  _update2(String dbName,String table,Map<String, dynamic> values) async {
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
  
    Database db = await openDatabase(path);
    int count = await db.update(table, values, where: 'name = ?', whereArgs: [values['name']]);//修改条件，对应参数值
    await db.close();
    if (count > 0) {
      setState(() {
        _data = "更新数据库操作完成，该sql删除条件下的数目为：$count";
  ...
  _query2(String dbName,String table) async {
    var databasesPath = await getDatabasesPath();
    String path = join(databasesPath, dbName);
