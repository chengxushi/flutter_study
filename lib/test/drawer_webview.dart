...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  bool _isTop = false; //滑动的网页是否到顶
  bool _againInto = false; //是否是再次进入
  bool _canScrolling = true;
  ...
  void initState() {
    ...
    _ac = new AnimationController(
      ...
      ..addListener(() {
        ...
        //如果panel没有完全打开时, 禁止网页的滚动
        //设边界值为0.9是因为要提前将网页的滚动开启, 避免当完全打开时, 第一次滚动没有效果
        if (_ac.value >= 0.99 && !_canScrolling) {
          setState(() {
            _canScrolling = true;
          });
        }
...
//    if(_isPanelOpen &&  _sc.hasClients && _sc.offset <= 0){
    ...
    if (dy < 0) {
      ...
      if (_againInto) {
        ...
        print('========_onGestureSlide===========再一次');
      }

      //第一次打开的时候, 要将_scrollingEnabled设为true, 这样第一次向上滑的时候才不会也将页面一起向下拉
      if (_isPanelOpen && !_scrollingEnabled) {
        setState(() {
          _scrollingEnabled = true;
        });
        print('========_onGestureSlide===========设置不给他动');
      }
    }
  }
